...
function classifyPanelHost(hostname: string): XuiHealthResult['panelHostKind'] {
...
const dayMilliseconds = daySeconds * 1000;

function normalizeExpiry(expiryTime: number | undefined): string | null {
...
function getActivationExpiryDays(client: XuiInboundClient): number | null {
...
function getStatus(
	...
	if (totalBytes === null) {
		return enabled ? ('active' as const) : ('disabled' as const);
	}
...
function summarizeClients(clients: VpnClientUsage[]): VpnDashboardSummary {
...
function summarizeInbounds(clients: VpnClientUsage[]): VpnInboundSummary[] {
