<script lang="ts">
	...
	type CreateMode = 'seller' | 'manager';
	type Props = {
		mode: CreateMode;
		data: {
		...
		stateTarget: any;
		createCommand: any;
		createTemplateCommand: any;
		deleteTemplateCommand: any;
	...
	const isManager = $derived(mode === 'manager');
	...
	const hasEnoughBalance = $derived(isManager || data.stats.gbBalance >= quotaGb);
	...
	const statItems = $derived([
		{
			label: isManager ? 'اعتبار مالک' : 'موجودی GB',
			value: isManager ? 'نامحدود' : data.stats.gbBalance.toLocaleString('fa-IR-u-nu-latn'),
			unit: isManager ? '' : 'GB',
			delta: isManager ? 'مصرفی از فروشندگان کم نمی‌شود' : 'اعتبار قابل مصرف برای فروش',
		...
		},
		{
			...
			unit: 'GB',
			delta: isManager ? 'فقط روی کانفیگ مشتری اعمال می‌شود' : 'از موجودی کم می‌شود',
			deltaTone: 'muted' as const
		},
		{
			label: isManager ? 'باقی‌مانده مالک' : 'باقی‌مانده بعد ساخت',
			value: isManager
				? 'نامحدود'
				: Math.max(0, data.stats.gbBalance - (Number.isFinite(quotaGb) ? quotaGb : 0)).toLocaleString(
						'fa-IR-u-nu-latn'
					),
			unit: isManager ? '' : 'GB',
			delta: 'محاسبه لحظه‌ای',
			deltaTone: hasEnoughBalance ? ('success' as const) : ('danger' as const)
		},
		{
			label: 'مدت کانفیگ',
	...
	function validateQuota() {
		...
		if (!isManager && data.stats.gbBalance < quotaGb) {
	...
	async function handleCreate() {
		...
		try {
			...
			const error = result?.createError ?? result?.adminCreateError;
			if (error) {
				toast.error(String(error));
				return;
			}

			if (result?.createSuccess || result?.adminCreateSuccess) {
				toast.success(
					isManager ? 'کانفیگ مدیر ساخته شد.' : 'کانفیگ ساخته شد و از موجودی GB شما کسر شد.'
				);
				customerLabel = '';
				internalNote = '';
				await onCreated?.();
	...
	async function handleDeleteTemplate(id: number) {
		try {
			const result = (await deleteTemplateCommand({ id }).updates(stateTarget)) as Record<string, unknown> | null;
			if (result?.templateSuccess) {
				toast.success(result.templateSuccess as string);
	...
	{:else}
		<div class="create-grid">
			<section class="va-card order-card">
				...
				<p class="hint">
					{isManager
						? 'مدیر مالک سیستم است؛ حجم فقط روی کانفیگ ساخته‌شده اعمال می‌شود و از موجودی فروشندگان کم نمی‌شود.'
			...
			<aside class="va-card summary-card">
				...
				<div class="summary-list">
					...
					<div>
						<span>{isManager ? 'موجودی مالک' : 'موجودی بعد ساخت'}</span>
						<strong>{isManager ? 'نامحدود' : `${Math.max(0, data.stats.gbBalance - (quotaGb || 0))} GB`}</strong>
					</div>
				</div>
				<button
					type="button"
					class="admin-btn admin-btn-primary full-width"
					disabled={!canCreate || createCommand.pending > 0}
					onclick={handleCreate}
				>
					<AnimatedIcon name="check" size={13} />
					<span>{createCommand.pending > 0 ? 'در حال ساخت...' : 'ساخت کانفیگ'}</span>
				</button>
			</aside>
		</div>
	{/if}
